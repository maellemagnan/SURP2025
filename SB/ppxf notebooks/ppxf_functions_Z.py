...
def run_ppxf(lam, fwhm, velscale, data, noise, redshift):
    """
    ...
    start = [[vel0, 120]] * 2  # 3 components (stars + 2 gas)

    # --- Components
    component = np.array([0] * n_stellar + [1, 1])  # both lines same gas comp
    moments   = [2, 2]   # stars + gas


    # --- Run pPXF
    pp = ppxf(
        templates, galaxy, noise, velscale, start,
        moments=moments, degree=-1, mdegree=10,   # smaller poly
